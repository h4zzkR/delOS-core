...
class SemanticTagger(ModuleUnit):
    ...
    def _encode_tokenize(self, ft, seq):
        tokenized = encode_dataset(ft, seq).tolist()
        encoded = ft.encode(
            tokenized, output_value='token_embeddings', convert_to_numpy=True, is_pretokenized=True
        )
        return encoded
...
if __name__ == "__main__":
    ...
    # inp_str = 'turn off the light in the kitchen'
    # _, inp, _ = ft.featurize(inp_str)
    # print(obj.tag(inp))
    # input()